...
friend class SimbiceImpl;

private:

	Ice::CommunicatorPtr communicator;
	Simbice::SimbiconServerPrx server;
	Simbice::ClientCallbackPrx client;
...
class SimbiceImpl : public Simbice::SimbiconServer {
public:
	ODEWorld *world;
public:
...
ODEWorld::ODEWorld() : AbstractRBEngine(){
	...
	Ice::ObjectAdapterPtr adapter = communicator->createObjectAdapterWithEndpoints("Simbice", "tcp -h localhost -p 10000");
	adapter->add(new SimbiceImpl(this), communicator->stringToIdentity("simbice"));
	adapter->activate();
	client = 0;
...
void ODEWorld::restoreState(const Simbice::AllState &state) {
	...
	for (auto it = state.jointStates.begin(); it != state.jointStates.end(); it++) {

		Simbice::JointState js = *it;
		Joint *j = jts[js.id];

		j->torque.x = js.torque.x;
		j->torque.y = js.torque.y;
		j->torque.z = js.torque.z;

	}

}

void ODEWorld::saveState(Simbice::AllState &state) {
	...
	for (int i=0;i<jointFeedbackCount;i++) {
		...
		state.contactPoints.push_back(scp);
	}
