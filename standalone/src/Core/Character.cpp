...
void Character::setState(DynamicArray<double>* state, int start){
	...
	//kinda ugly code....
	Vector3d tmp = rs.getPosition();
	af->root->state.position.x = tmp.x;
	af->root->state.position.y = tmp.y;
	af->root->state.position.z = tmp.z;
...
void Character::setHeading(double val, DynamicArray<double>* state, int start){
	Vector3d _010 = Vector3d(0,1,0);
	setHeading(Quaternion::getRotationQuaternion(val, _010 ), state, start);
}
...
void Character::setHeading(double val){
	...
	getState(&state);
	Vector3d _010 = Vector3d(0,1,0);
	setHeading(Quaternion::getRotationQuaternion(val, _010), &state);
	setState(&state);
}
